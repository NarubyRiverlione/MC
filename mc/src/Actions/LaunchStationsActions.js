...
export function Select(caption) {
  const station = caption === Cnst.LaunchStations.Numbers.one || Cnst.LaunchStations.Numbers.two
...
export function ShowErrorStatus(status) {
  AppDispatcher.dispatch({
    type: ActionCnst.LaunchStations.ShowErrorStatus,
    payload: status,
  })
}
